...
def _withinPotentialColumnRange(detectedText, course_id_line, LEdge, REdge):
    ...
    if ("celdt" in detectedText['text'].lower()):
        print(f"!!!CELDT: {detectedText['text']}")
        print(f"[0]x = {detectedText['bounding_box'][0]['x']}")
        print(f"[0]y = {detectedText['bounding_box'][0]['y']}")
        print(f"course id line: {course_id_line}")
        print(f"left column edge: {LEdge}")
        print(f"right column edge: {REdge}")
